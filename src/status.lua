...
-- if window width <= 50 then make the statusline smaller
    ...
    callback = function(args)
        ...
        if width >= 50 then
            vim.api.nvim_buf_set_option(args.buf, 'statusline', "%#modeGroup#%{v:lua.statusLineMode()}%*%#tri1#%*%#fileGroup# %{v:lua.languageIcon()} %f %*%#tri2#%*%#midGroup# %m%h%r%w%=%#tri3#%*%#rulerGroup# %l : %c %*%#tri4#%*%#percentGroup# %p%%/%L %*")
        else
